...
void drawKnot() {
    ...
    for (float t = 0; t < 10 * PI; t += 0.01) {
        float y = cos(2*t) * (3+cos(5*t));
        float z = sin(5*t);
        float x = sin(2*t) * (3+cos(5*t));      
        glVertex3f(x, y, z);
    }
    glEnd();
}
...
int main(int argc, char** argv) {
    glutInit(&argc, argv);
    glutInitWindowSize(1000, 1000); /* you can change this if you want different resolution */
    ...
    glLoadIdentity();
    gluPerspective(80, 1.0, 1.0, 10.0);
